...
Page({
    ...
    click_row: function(t) {
        ...
        this.setData({
            ...
            box_idx: a,
            tmp_weight: e.accept_weight ? e.accept_weight / 1e3 : e.weight / 1e3,
            tmp_amount: e.accept_amount ? e.accept_amount : e.amount,
    ...
    click_edit: function() {
        if (this.data.box_idx < 0) t.alert("提示", "缺少修改目标"); else if ("" == this.data.box_weight || isNaN(this.data.box_weight) || this.data.box_weight <= 0) t.alert("提示", "请输入正确的质量"); else if (this.data.box_show_amount && ("" == this.data.box_amount || isNaN(this.data.box_amount) || this.data.box_amount < 1)) t.alert("提示", "请输入正确的数量"); else if (this.data.collect[this.data.box_idx]) {
